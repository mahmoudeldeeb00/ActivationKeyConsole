...
Console.WriteLine($"1 Encryption , 2 Decryption , 3 Create Key , 4 Read Key , 5 Accpet Key ");
try
{
    ...
    switch (resultInt)
    {
        ...
        case 3:
            ...
            Console.WriteLine($"Insert Company Name  , Default : ({model.CompanyName} ) Press Enter To Take Default Value");
            resultString = Console.ReadLine().Trim();
            model.CompanyName = resultString;
            Console.WriteLine($"Value Is ::: {model.CompanyName}");
            Console.WriteLine("----------------------------------------------------------------------------------------------------------------------------");

            Console.WriteLine($"Insert Company Tax Registration Number  , Default : ({model.TaxNumber} ) Press Enter To Take Default Value");
            resultString = Console.ReadLine().Trim();
            model.TaxNumber = resultString;
            Console.WriteLine($"Value Is ::: {model.TaxNumber}");
            Console.WriteLine("----------------------------------------------------------------------------------------------------------------------------");
            ...
            Console.WriteLine($"Enter Y For Accept Key Or Enter N For Not Accept  , Default : (Not Accept) Press Enter To Take Default Value");
            resultString = Console.ReadLine().Trim();
            model.IsAccepted = resultString.ToUpper() == "Y" ? true : false;
...
Company Name : ({hashedModel.CompanyName}),
Tax Number : ({hashedModel.TaxNumber}),
IsAccepted : ({(hashedModel.IsAccepted ? "Accepted" : "Not Accepted")})
"
                );
            break;

        case 5:
            Console.WriteLine($"Enter Activation Key  That You Want to Accepted It");
            var readerKey = Console.ReadLine().Trim();
            var hashedModelKey = JsonConvert.DeserializeObject<AppSettingClass>(DecryptText.DecryptPlainText(readerKey).Result);
            hashedModelKey.IsAccepted = true;
            Console.WriteLine("----------------------------------------------------------------------------------------------------------------------------");

            Console.WriteLine($"New Activation Key After Accepted It :");
            Console.WriteLine("----------------------------------------------------------------------------------------------------------------------------");
            Console.WriteLine(EncryptText.EncryptPlainText(JsonConvert.SerializeObject(hashedModelKey)));
            Console.WriteLine("----------------------------------------------------------------------------------------------------------------------------");
